...
class SocialSharing_Shares_Module extends SocialSharing_Core_BaseModule
{
    ...
    public function checkWhetherNeedToSaveShare()
    {
        $shareModel = $this->getModelsFactory()->get('shares');

        if (! $shareModel->getIsEnableOption() || ! $shareModel->getSharesLogOption())
        {
            return false;
        }
    ...
    public function checkWhetherNeedToSaveShows()
    {
        $shareModel = $this->getModelsFactory()->get('shares');

        if (! $shareModel->getIsEnableOption() || ! $shareModel->getViewsLogOption())
        {
            return false;
        }
    ...
    public function saveViews(SocialSharing_Projects_Project $project)
    {
        ...
        if ($this->checkWhetherNeedToSaveShows())
        {
            try {
