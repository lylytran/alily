<?php
/**
 * Brunch Pro Featured Posts Widget display markup.
 *
...
if ( have_posts() ) :
	...
	if ( $grid = brunch_pro_grid_exists( $data['simple_grid'] ) ) {
		add_filter( 'post_class', "brunch_pro_grid_{$grid}" );
	}
	...
	while ( have_posts() ) : the_post();
		...
		$image = sprintf( '<a href="%s" title="%s" class="%s">%s</a>',
			get_permalink(),
	...
	if ( $grid ) {
		remove_filter( 'post_class', "brunch_pro_grid_{$grid}" );
	}
