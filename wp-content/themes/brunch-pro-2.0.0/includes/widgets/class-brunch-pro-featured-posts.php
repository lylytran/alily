...
/**
 * Brunch Pro Featured Post widget class.
 *
 * @since   1.0.0
 * @package BrunchPro\Widgets
 */
class Brunch_Pro_Featured_Posts extends WP_Widget {
	/**
	...
	public function __construct() {
		$this->defaults = array(
			...
			'show_title'              => 0,
			'show_byline'             => 0,
			'post_info'               => '[post_date] ' . __( 'By', 'brunch-pro' ) . ' [post_author_posts_link] [post_comments]',
			'show_content'            => 'excerpt',
			'content_limit'           => '',
			'more_text'               => __( 'Read More', 'brunch-pro' ),
			'more_from_category'      => '',
			'more_from_category_text' => __( 'More Posts from this Category', 'brunch-pro' ),
		);

		$widget_ops = array(
			'classname'   => 'featured-content featuredpost',
			'description' => __( 'Displays featured posts with thumbnails', 'brunch-pro' ),
		);
		...
		parent::__construct( 'featured-post', __( 'Brunch Pro - Featured Posts', 'brunch-pro' ), $widget_ops, $control_ops );
	}
	 ...
	 * @since  1.0.0
	 * @access public
	 * @return string Brunch_Pro_Featured_Posts::$more_text the read more text for the current widget.
	 */
	...
	protected function the_entry_content( $data ) {
		...
		add_filter( 'brunch_pro_read_more_text', array( $this, 'get_read_more_text' ) );
		...
		remove_filter( 'brunch_pro_read_more_text', array( $this, 'get_read_more_text' ) );
	}
	...
	public function widget( $args, $instance ) {
		...
		$this->get_widget_template( 'display-brunch-pro-featured-posts', $data, true );
	}
	...
	public function form( $instance ) {
		...
		$this->get_widget_template( 'form-brunch-pro-featured-posts', $instance );
	}
}
